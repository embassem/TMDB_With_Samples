...
extension NetworkManager {
    ...
    func getActors(completion: @escaping (
        _ result: Swift.Result<HomeProduct, NetworkError>,
        _ statusCode: StatusCode?
        ) -> Void) {
        
        provider.request(MultiTarget(ProductService.readProducts)) { (result) in
            switch result {
            case .success(let response):
                if (200...299 ~= response.statusCode) {
                    do {
                        let result = try JSONDecoder().decode(HomeProduct.self, from: response.data)
